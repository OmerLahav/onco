...
class MailSendHelper {
  ...
  public static function send_email($email_data = array(),$to_sender_emailids = array(),$cc_emails_send_list = array(),$bcc_emails_send_list = array()) {
    ...
    $email_data->template_body =  str_replace($to_array,$replace_array,$email_data->template_body);
    ...
    if(count($to_sender_emailids)){
      ...
      Mail::send('emails.send',$data = array('email_body'=>$email_data->template_body), function ($message) use ($email_data,$to_sender_emailids,$cc_sender_emailids,$bcc_sender_emailids) {
              $message->subject($email_data->subject);
              $message->from($email_data->from_email_id,SITE_NAME);
